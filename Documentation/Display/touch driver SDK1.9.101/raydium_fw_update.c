		...
		unsigned char u8_type)
{
	...
	switch (u8_type) {
	case RAYDIUM_INIT:
		u32_fw_size = RAYDIUM_PDA_INITLENGTH;
	...
	case RAYDIUM_BOOTLOADER:
		u32_fw_size = RAYDIUM_PDA_BOOTLENGTH;
	...
	if (u8_buf[2] >= 4) {
		...
		if (u8_type == RAYDIUM_FIRMWARE) {
			...
			if (i32_ret < 0)
				...
				/* unlock PRAM */
				u8_buf[0] = 0x0E;
				i32_ret = raydium_i2c_pda_write(client, RAYDIUM_PDA_PRAMLOCK,
								u8_buf, 4);
				if (i32_ret < 0)
					goto exit_upgrade;
		 ...
		 */
		if (wait_fw_state(client, RAYDIUM_PDA_BOOTENG3, 6, 2000,
				 u16_retry) == ERROR) {
		...
		/*#confirm in burn mode*/
		if (wait_fw_state(client, RAYDIUM_PDA_PRAMLOCK, 63,
				 2000, u16_retry) == ERROR) {
			pr_err("[touch]Error, confirm in burn mode\n");
			i32_ret = ERROR;
			goto exit_upgrade;
		...
		*/
		...
		if (wait_fw_state(client, RAYDIUM_PDA_BOOTSTATE, 161,
				 2000, u16_retry) == ERROR) {
		...
		if (wait_fw_state(client, RAYDIUM_PDA_BOOTENG4, 163,
		 ...
		 */
		if (wait_fw_state(client, RAYDIUM_PDA_BOOTENG4,
		...
		if (wait_fw_state(client, RAYDIUM_PDA_BOOTSTATE,
		...
		memset(u8_buf, 0, sizeof(u8_buf));
		u8_buf[0] = 0xa5;
		i32_ret = raydium_i2c_pda_write(client, RAYDIUM_PDA_BOOTENG4, u8_buf, 4);
		if (i32_ret < 0)
			goto exit_upgrade;
		 ...
		 */
		if (wait_fw_state(client, RAYDIUM_PDA_BOOTENG3,
...
int raydium_burn_fw(struct i2c_client *client)
{
	...
	if ((g_raydium_ts->id & 0x2000) != 0) {
		...
		i32_ret = raydium_fw_upgrade_2X(client, RAYDIUM_BOOTLOADER, DISABLE);
		if (i32_ret < 0)
			goto exit_upgrade;
